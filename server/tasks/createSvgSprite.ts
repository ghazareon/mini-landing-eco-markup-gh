...
async function svgSprite() {
	...
	let formatingSvgTw = await prettier.format(
		`${svgTwStart} ${svgTwBody} ${svgTwEnd}`,
		{
			parser: "scss",
		},
	);
	...
	fs.writeFile(
		`${svgPaths.styleOutDir}/sprite-${svgDirName}.scss`,
		formatingSvgTw,
	);
	...
	fs.writeFile(
		`${svgPaths.outDir}/use-sprite-${svgDirName}.html`,
		formatingUseSvg,
	);
